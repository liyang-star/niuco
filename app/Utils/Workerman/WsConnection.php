...
class WsConnection
{
    ...
    protected function onMarketKline($con, $data, $match)
    {
        ...
        $match_key = $currency_match->currency_name . '.' . $currency_match->legal_name;
        $obj_price = $redis->get($match_key.".set_price");
       // $obj_price = $redis->get("BTC.USDT.set_price");
        echo "--------------match_key:" . $match_key ."------------obj_price:".$obj_price . PHP_EOL;
        $open_price = sctonum($tick->open);
        $close_price = sctonum($tick->close);
        $high_price = sctonum($tick->high);
        $low_price = sctonum($tick->low);
        $vol = sctonum($tick->vol);
        $amount = sctonum($tick->amount);

        if($obj_price>0 && $obj_price<200){
            echo "----------------------原close_price :" . $close_price . PHP_EOL;
            //$open_price =  $open_price + $open_price * $obj_price;
            $close_price = $close_price + $obj_price;
            $high_price = $high_price + $obj_price;
           // $low_price = $low_price + $low_price * $obj_price;
            //$vol = $vol + $vol * $obj_price;
          //  $amount = $amount + $amount * $obj_price;
           echo "-----------------------拉升后Close_price：". $close_price . PHP_EOL;
        }elseif($obj_price>-200 && $obj_price<0){
            echo "----------------------原close_price :" . $close_price . PHP_EOL;
            $close_price = $close_price + $obj_price;
            //$high_price = $high_price + $high_price * $obj_price;
            $low_price = $low_price + $obj_price;
            echo "-----------------------打压后Close_price：". $close_price . PHP_EOL;
        }else{
          //  echo "----------------------不修正\n";
        }
