...
class CurrencyController extends Controller
{
    ...
    public function postAdd(Request $request)
    {
        ...
        try {
            ...
            $save['address_omni']=$address_omni;
            $save['address_erc']=$address_erc;
            $save['micro_trade_fee']=$micro_trade_fee;
            $save['issue_num']=$issue_num;
            $save['content']=$content;
            $save['name']= $name;
            $save['sort']= intval($sort);
            $save['logo']= $logo;
            $save['is_legal']= $is_legal;
            $save['is_lever']= $is_lever;
            $save['is_match']= $is_match;
            $save['is_micro'] = $is_micro;
            $save['min_number'] = $min_number;
            $save['max_number']= $max_number;
            $save['micro_holdtrade_max']= $micro_holdtrade_max;
            $save['rate']= $rate;
            $save['price']= $price;
            $save['micro_min'] = $micro_min;
            $save['micro_max'] = $micro_max;
            $save['rmb_relation'] = $rmb_relation;
            $save['decimal_scale'] = $decimal_scale;
            $save['insurancable']= $insurancable;
            $save['chain_fee'] = $chain_fee; //链上手续费
            $save['contract_address'] = $contract_address;
            $save['type']= $type;
            $save['is_display'] = 1;
            if($id>0){
                DB::table('currency')->where(array('id'=>$id))->update($save);
            }else{
                DB::table('currency')->insert($save);
            }
            DB::commit();
            return $this->success('操作成功');
        } catch (\Exception $exception) {
