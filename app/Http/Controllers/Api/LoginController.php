...
class LoginController extends Controller
{
    ...
    // type 1普通密码 2手势密码 testa
        ...
        foreach($ress as $order){
            ...
            // if($status==1 && $is_cancel==0){
            //     echo 123;exit;
            // }
            
            // echo $status.'-'.$is_cancel.'<br>';
            if($status==1 && $is_cancel==0){
                //  echo $order->id.'-'.'<br>';exit;
                LhDepositOrder::dispatchInterest($order->id);
            //var_dump($a);
                if($order->end_at < date('Y-m-d')){
                    //todo 时间到了 释放存款
                    LhDepositOrder::where('id',$order->id)->update(['status' => 2,'last_settle_time'=>date("Y-m-d")]);
                }else{
                    LhDepositOrder::where('id',$order->id)->update(['last_settle_time'=>date("Y-m-d")]);
                }
            }else{
                LhDepositOrder::where('id',$order->id)->update(['status' => 2,'last_settle_time'=>date("Y-m-d")]);
            }
