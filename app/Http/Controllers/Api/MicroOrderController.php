...
class MicroOrderController extends Controller
{
    ...
    public function getPayableCurrencies()
    {
        ...
        $array=json_decode(json_encode($currencies), true);
        $micro_balance=0;
        foreach ($array as $k=>$v){
            if($v['name']=='USDT'){
                $micro_balance=$v['user_wallet']['micro_balance'];
            }
        }
         foreach ($array as $k=>$v){
            $array[$k]['user_wallet']['micro_balance']=$micro_balance;
            $array[$k]['user_wallet']['micro_with_insurance']=$micro_balance;
            $array[$k]['total_account']=$micro_balance;
        }
        // echo '<pre>';
        //     var_dump($array);exit;
